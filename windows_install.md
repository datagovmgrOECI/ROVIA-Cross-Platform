...
### 3. Install FFmpeg (for cut scripts)

FFmpeg is **not required to run ROVIA** — moviepy bundles its own FFmpeg binary.
FFmpeg is required only if you want to run the `rovia_cut_clips.bat` script generated by dry-run mode.
...
Or download manually: https://ffmpeg.org/download.html (add to PATH after installing)

### 4. Download ROVIA Model File
